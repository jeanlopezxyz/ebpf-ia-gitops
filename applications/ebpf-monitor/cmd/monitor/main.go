...
// Helper functions to handle missing generated code
func loadNetworkSpec() (*ebpf.CollectionSpec, error) {
    // This would normally be generated by bpf2go
    // If generation fails, return error to trigger fallback mode
    return nil, fmt.Errorf("eBPF code not generated - run go generate ./...")
}

func isRingbufClosed(err error) bool {
    // Simple check for closed ringbuf error
    return err != nil && (err.Error() == "closed" || err.Error() == "reader closed")
}
